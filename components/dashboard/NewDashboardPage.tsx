...
//                 key={item.label}
//                 href={item.href}
//                 className={`flex flex-col items-center gap-0.5 px-5 py-2 text-[13px] transition-colors no-underline border-b-2 ${
//                   item.active
...
//             <button className="border border-gray-300 bg-white rounded-full px-4 py-1.5 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors">
//               Become a tutor
//             </button>
//             <div className="flex items-center gap-1 cursor-pointer">
//               <div className="w-8 h-8 rounded-full bg-indigo-700 text-white flex items-center justify-center text-[13px] font-semibold">
//                 O
//               </div>
//               <svg
...
//         {/* Actions */}
//         <p className="text-[13px] text-gray-400 mb-3">Actions</p>
...
//         {/* Learning Overview */}
//         <p className="text-[13px] text-gray-400 mb-3">Learning Overview</p>
//         <div className="grid grid-cols-4 gap-3 mb-7">
//           {stats.map((s) => (
//             <div key={s.label} className="bg-white border border-gray-200 rounded-xl p-4">
//               <p className="text-[12px] text-gray-400 mb-2">{s.label}</p>
...
//               <span className="text-[13px] font-medium text-gray-800">Managed Bookings</span>
//               <Link href="/bookings" className="text-[12px] text-indigo-700 flex items-center gap-0.5 no-underline hover:underline">
...
//                     <th
//                       key={h}
//                       className="text-left text-[12px] text-gray-400 font-medium px-4 py-2.5 border-b border-gray-100"
//                     >
//                       {h}
//                     </th>
//                   ))}
...
//           <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
//             <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-100">
//               <span className="text-[13px] font-medium text-gray-800">Messages</span>
//               <Link href="/chat" className="text-[12px] text-indigo-700 flex items-center gap-0.5 no-underline hover:underline">
//                 Go to chat
//                 <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
//                   <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
//                 </svg>
//               </Link>
//             </div>
//             <p className="px-4 pt-3 pb-1 text-[12px] text-gray-300">Chat</p>
//             {messages.map((m) => (
//               <div
//                 key={m.id}
...
//                   <p className="text-[12px] text-gray-400 truncate">{m.preview}</p>
//                 </div>
//                 <div className="flex flex-col items-end gap-1 flex-shrink-0">
//                   <span className="text-[11px] text-gray-400">{m.time}</span>
//                   {m.unread > 0 && (
//                     <span className="w-[18px] h-[18px] rounded-full bg-indigo-700 text-white text-[10px] flex items-center justify-center">
//                       {m.unread}
//                     </span>
//                   )}
//                 </div>
...
//         {/* Resource & Support */}
//         <p className="text-[13px] text-gray-400 mb-3">Resource & Support</p>
//         <div className="grid grid-cols-3 gap-3">
//           <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 relative overflow-hidden">
//             <p className="text-[14px] font-semibold text-gray-800 mb-1">Watch our demo video</p>
//             <p className="text-[12px] text-gray-500 mb-4 max-w-[160px]">
...
//             <p className="text-[14px] font-semibold text-gray-800 mb-1">Learn about our tutors</p>
//             <p className="text-[12px] text-gray-500 mb-4">Watch this intro video to learn more about our tutors</p>
//             <WatchButton />
//           </div>

//           <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
//             <p className="text-[14px] font-semibold text-gray-800 mb-1">What is a finder's fee</p>
//             <p className="text-[12px] text-gray-500 mb-4">Watch this intro video to learn about our finder's fee</p>
//             <WatchButton />
//           </div>
//         </div>
//       </div>
...
//       </div>
//       <div>
//         <p className="text-[14px] font-semibold text-gray-800 mb-0.5">{title}</p>
...
//   return (
//     <button className="inline-flex items-center gap-1.5 border border-gray-300 rounded-full px-3.5 py-1.5 text-[12px] text-gray-700 bg-white hover:bg-gray-50 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-white text-[#1E1E1E]">
      <header className="sticky top-0 z-50 border-b border-[#E8EAF1] bg-white shadow-[0_2px_10px_rgba(33,38,79,0.08)]">
        <div className="grid h-[58px] w-full grid-cols-[auto_1fr_auto] items-center gap-4 px-4 sm:px-5">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`relative flex h-full flex-col items-center justify-center gap-[2px] px-2.5 text-[15px] font-semibold leading-[150%] transition-colors ${
                    item.active
                      ? "text-[#4A46D6]"
                ...
                >
                  <Icon
                    strokeWidth={1.9}
                    className={`h-[13px] w-[13px] ${item.active ? "text-[#7073EA]" : "text-[#A6ADBD]"}`}
                  />

                  <span>{item.label}</span>

                  {item.active && (
                    <span className="absolute bottom-0 h-[2.5px] w-[68px] rounded-full bg-[#4B49D8]" />
                  )}
                </Link>
              );
            })}
          ...
          <div className="flex items-center justify-end gap-2.5">
            <button className="rounded-full border border-[#E6E8EF] bg-[#F2F3F7] px-3.5 py-[6px] text-[11px] font-semibold text-[#454B5D] transition hover:bg-[#EBEDF3]">
              Become a tutor
            </button>

            <button className="flex items-center gap-1 rounded-full bg-[#F1F2F6] px-1 py-1">
              <div className="flex h-6.5 w-6.5 items-center justify-center rounded-full bg-[#221D71] text-[11px] font-semibold text-white">
                O
              </div>
      ...
      <main className="mx-auto max-w-[1240px] px-3 py-3 sm:px-5">
        ...
        <div className="mt-3">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="rounded-[10px] border border-[#E5E7EB] bg-white px-4 py-3"
              >
                <p className="text-[10px] leading-[150%] text-[#9CA3AF]">{stat.label}</p>

                <h3 className="mt-1 text-[20px] font-semibold leading-[120%] tracking-[-0.2px] text-[#111827]">
                  {stat.value}
                </h3>
              </div>
            ))}
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
          <div className="overflow-hidden rounded-2xl border border-[#E4E7F0] bg-white shadow-[0_8px_24px_rgba(27,35,74,0.04)]">
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-[#F4F6FA] text-[12px] leading-[150%] text-[#848BA0]">
              ...
              <tbody>
                {bookings.map((booking, index) => (
                  <tr
                    key={index}
                    className="border-t border-[#EEF1F6] text-[14px] leading-[150%] font-normal text-[#4F566A]"
                  >
                    <td className="px-4 py-[10px]">{booking.tutor}</td>
                    <td className="px-4 py-[10px]">{booking.subject}</td>
          ...
          <div className="overflow-hidden rounded-2xl border border-[#E4E7F0] bg-white shadow-[0_8px_24px_rgba(27,35,74,0.04)]">
            ...
            <div className="border-b border-[#EEF1F6] px-4 py-2 text-[13px] font-medium leading-[150%] text-[#B1B7C6]">
              Chat
            </div>
            ...
            <div>
              {messages.map((message, index) => (
                ...
                >
                  <div
                    className="flex h-[34px] w-[34px] items-center justify-center rounded-md text-[12px] font-semibold"
                    style={{
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[13px] leading-[150%] font-semibold text-[#374151]">
                      {message.name}
                    </p>

                    <p className="truncate text-[11px] leading-[150%] text-[#9CA3AF]">
                      {message.message}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="flex h-[16px] w-[16px] items-center justify-center rounded-full bg-[#4338CA] text-[9px] text-white">
                      1
                    </div>
                  </div>
                </div>
...
function SectionTitle({ title }: { title: string }) {
  return (
    <p className="mb-2.5 text-[12px] font-medium leading-[150%] text-[#9CA3AF]">{title}</p>
  );
}
...
function CardHeader({
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5">
      <h3 className="text-[13px] font-normal leading-[150%] text-[#4B5563]">{title}</h3>

      <button className="flex items-center gap-1 text-[11px] font-medium leading-[150%] text-[#4F46E5] hover:opacity-80">
        {action}
...
function ActionCard({
  ...
  return (
    <div
      className={`${bg} ${border} flex items-center gap-3 rounded-[10px] border px-4 py-3 transition hover:shadow-sm`}
    >
      <div
      ...
      <div>
        <h3 className="text-[13px] font-semibold leading-[150%] text-[#374151]">{title}</h3>

        <p className="mt-0.5 text-[11px] leading-[150%] text-[#9CA3AF]">{subtitle}</p>
      </div>
    </div>
  );
}
...
function SupportCard({
  ...
  return (
    <div
      className={`${bg} ${border} relative overflow-hidden rounded-[14px] border p-4`}
    >
      <h3 className="max-w-[220px] text-[16px] font-semibold leading-snug text-[#374151]">
        {title}
      </h3>

      <p className="mt-1.5 max-w-[230px] text-[12px] leading-[150%] text-[#6B7280]">
        {description}
      </p>

      <button className="mt-4 inline-flex items-center gap-2 rounded-full border border-[#D1D5DB] bg-white px-4 py-1.5 text-[11px] font-medium text-[#4B5563] transition hover:bg-[#FAFAFA]">
      ...
      {large && (
        <div className="absolute bottom-3 right-4">
          <div className="relative flex h-[90px] w-[130px] items-center justify-center rounded-xl border border-[#D1D5DB] bg-white shadow-sm">
